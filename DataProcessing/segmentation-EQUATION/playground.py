...
kernel_22 = np.ones((2,2),dtype=np.uint8)
...
def remove_slash(im):
    ...
    ret, im = cv2.threshold(im,200,255,cv2.THRESH_BINARY_INV)
    ...
    out4 = cv2.dilate(out4, kernel_22, iterations=1)
    out4 = cv2.erode(out4, kernel_22, iterations=1)
...
def img_preprocessing(img_dir):
    ...
    img_list = [x for x in img_list if '.jpg' in x and x[0] != '.']
    ...
    for index in range(len(img_list)):
        ...
        im_denoise = denoise(im_no_slash, area_threshold=100)

        # erosion and dilation
        # 3 steps -> remain the walls with double-line representation
        im_preprocess = cv2.dilate(im_denoise, kernel, iterations=4)
        im_preprocess = cv2.erode(im_preprocess, kernel, iterations=7)
        im_preprocess = cv2.dilate(im_preprocess, kernel, iterations=10)
        im_preprocess = cv2.erode(im_preprocess, kernel, iterations=7)

        # first round denoise -> remove dots
        im_denoise_2 = denoise(im_preprocess, area_threshold=5000)

        # write images into the output directory
        cv2.imwrite('./output/'+str(img_list[index]), im_denoise_2)

img_preprocessing('./Image')